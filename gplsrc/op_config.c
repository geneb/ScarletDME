 ...
 *             This is a bug that was (and may still be!) in the original GPL release
 *             of OpenQM and may remain in the commercial codebase to this day.
 * 
...
void op_config() {
  ...
  */
  ...
  if (!strcmp(param, "CMDSTACK"))
    ...
    result.data.value = pcfg.qmclient_mode;
...
void op_pconfig() {
  ...
  */
  ...
  if (!strcmp(param, "CODEPAGE")) {
    ...
    GetInt(descr);
    if ((descr->data.value < pcfg.qmclient_mode) || (descr->data.value > 2))
      goto exit_op_pconfig;
